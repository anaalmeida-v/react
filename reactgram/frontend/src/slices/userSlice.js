...
export const updateProfile = createAsyncThunk("user/update", async (user, thunkAPI) => {
    ...
    const data = await userService.updateProfile(user, token)//user - resgata dados do usuário
...
export const getUserDetails = createAsyncThunk("user/get",async(id, thunkAPI) => {
    //não precisa ser feita a validação com o token pois já foi feita no back-end
...
export const userSlice = createSlice({
    ...
    extraReducers: (builder) => {//parte das execuções que são feitas na api//builder-construtor: cria ações separadamente
        builder
            .addCase(profile.pending, (state) => {//se a requisição foi enviada mas não obteve nenhuma resposta   
                state.loading = true
                state.error = null
            })
            .addCase(profile.fulfilled, (state, action) => {//significa que a operação foi concluída com sucesso.
                ...
                state.user = {}//está tentando cadastrar ou logar, mas não há usuário
            }).addCase(getUserDetails.pending, (state) => {//se a requisição foi enviada mas não obteve nenhuma resposta   
                state.loading = true
                state.error = null
            })
            .addCase(getUserDetails.fulfilled, (state, action) => {//significa que a operação foi concluída com sucesso.
                state.loading = false
                state.success = true
                state.error = null
                state.user = action.payload
