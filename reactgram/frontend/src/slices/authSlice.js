...
export const register = createAsyncThunk("auth/register",//exporta função e cria ela com createAsyncThunk, 1º argumento:nome, 2º argumento:função //nome segue convenção: auth(nome da entidade que está sendo trabalhada, autenticação) - register(nome atual)
    async (user, ThunkAPI) => {//recebe usuário e ThunkAPI(nos permite utilizar funções extras;ex.: para a execução e identificar um erro da api)
